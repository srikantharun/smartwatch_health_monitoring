...
def generate_model_performance_metrics(reference_data: pd.DataFrame, current_data: pd.DataFrame) -> Tuple[Dict, str]:
    ...
    try:
        ...
        logger.info(f"Initial reference columns: {reference_data.columns.tolist()}")
        logger.info(f"Initial current columns: {current_data.columns.tolist()}")

        # CRITICAL: Check for prediction column before any filtering
        has_prediction = 'prediction' in current_data.columns

        # Make sure reference data and current data have the same columns
        # BUT EXCLUDE 'prediction' and 'target' from this filtering
        essential_cols = ['prediction', 'target']
        non_essential_cols = [col for col in current_data.columns if col not in essential_cols]
        ref_non_essential = [col for col in reference_data.columns if col not in essential_cols]

        common_non_essential = list(set(non_essential_cols).intersection(set(ref_non_essential)))

        # Exclude problematic columns
        exclude_cols = ['user_id', 'user_name', 'device_id', 'device_type']
        use_cols = [col for col in common_non_essential if col not in exclude_cols] + \
                  [col for col in essential_cols if col in current_data.columns or col in reference_data.columns]

        # Keep only the columns we want
        current_data = current_data[[col for col in use_cols if col in current_data.columns]]
        reference_data = reference_data[[col for col in use_cols if col in reference_data.columns]]

        # Add logging to help debug
        logger.info(f"After filtering - reference columns: {reference_data.columns.tolist()}")
        logger.info(f"After filtering - current columns: {current_data.columns.tolist()}")

        # If we lost the prediction column but we had it initially, add it back
        if 'prediction' not in current_data.columns and has_prediction:
            logger.warning("Prediction column lost during filtering, adding it back")
            current_data['prediction'] = current_data_copy['prediction']

        # Ensure 'target' column exists in both datasets
        if 'target' not in current_data.columns:
            if 'actual' in current_data.columns:
                logger.info("Creating 'target' column from 'actual' in current data")
                current_data['target'] = current_data['actual']
            else:
                raise ValueError("No 'target' or 'actual' column in current data")

        if 'prediction' not in current_data.columns:
            # For model performance, we absolutely need predictions
            raise ValueError("No 'prediction' column in current data")

        if 'target' not in reference_data.columns:
            logger.info("Creating dummy 'target' column in reference data")
            reference_data['target'] = 0

        if 'prediction' not in reference_data.columns:
            logger.info("Creating dummy 'prediction' column in reference data")
            reference_data['prediction'] = 0

        # Final check of columns
        logger.info(f"Final reference columns: {reference_data.columns.tolist()}")
        logger.info(f"Final current columns: {current_data.columns.tolist()}")
